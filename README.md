...
Нужно оптимизировать actions в vuex
Запросы постоянно повторяются

###Что у нас есть?
get и post запросы,
Всегда есть url,
для post запроса есть data,
можно передать config для axios
Результат должен типизироваться
---
Можно сделать класс Req, методы которого будут возвращать сами себя
```new Req('get', 'api/birds').setConf({}).auth().send<type>()```
Тогда получается такой конструктор запросов, примерно, как в laravel

---
Можно сделать разные функции для get и post

```get<type>('api/birds', config, auth)```
или
```post<type>('api/birds', data, config, auth)```
---
